...
export default class CobrowseIO {
  ...
  static handleSessionRequest(session) {
    ...
    Alert.alert('Support Request', 'A support agent would like to use this app with you. Do you accept?', [{
      text: 'Reject',
      onPress: () => {
  ...
  static handleRemoteControlRequest(session) {
    ...
    Alert.alert('Remote Control Request', 'A support agent would like to take remote control of this app. Do you accept?', [{
      text: 'Reject',
      onPress: () => {
  ...
  static currentSession() {
    return CobrowseIONative.currentSession().then(session => session != null ? new Session(session) : null);
  }
  static createSession() {
    return CobrowseIONative.createSession().then(session => session != null ? new Session(session) : null);
  }
  static getSession(codeOrId) {
    return CobrowseIONative.getSession(codeOrId).then(session => session != null ? new Session(session) : null);
  }

  /** @deprecated */
  static activateSession() {
    return CobrowseIONative.activateSession().then(session => session != null ? new Session(session) : null);
  }
